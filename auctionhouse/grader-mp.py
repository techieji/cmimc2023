...
class AuctionHouseGrader:
    ...
    def __init__(self, num_tournaments, debug, lock) -> None:
        """
    ...
    def _run_single_tournament(self):
        strategies = get_strategies()
        curr_bidders = [(strategy.__name__, strategy) for strategy in strategies]
        bidder_scores = {bidder[0]: 0 for bidder in curr_bidders}
        with self.lock:
            if not hasattr(self, 'bidder_scores'):
                self.bidder_scores = deepcopy(bidder_scores)

        bidder_wallet = {bidder[0]: 100 for bidder in curr_bidders}
        bidder_history = {bidder[0]: [] for bidder in curr_bidders}
        bidder_losses = {bidder[0]: 0 for bidder in curr_bidders}
        ...
        while len(curr_bidders) > 1:  # Simulates rounds
            ...
            for loser in losers:
                bidder_scores[loser[0]] += 1/(1 + math.sqrt(len(curr_bidders)))

        if len(curr_bidders) == 1:
            bidder_scores[curr_bidders[0][0]] += 1
            if self.debug:
            ...
            if self.debug:
                print("No winner")
        with self.lock:
            for k, v in bidder_scores.items():
                self.bidder_scores[k] += v
    ...
    def grade(self) -> None:
        ...
        for i in range(self.num_tournaments):
            ...
            pool.apply(self._run_single_tournament)
        pool.close()
        pool.join()
    ...
    def print_result(self) -> dict:
        ...
        i = 1
        print("{:<5} {:<25} {:<12}".format("Rank", "Strategy", "Score"))
        for bidder in sorted_scores:
            print("{:<5} {:<25} {:<12}".format(i, bidder, sorted_scores[bidder]))
            i += 1
